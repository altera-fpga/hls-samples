...
This sample is an FPGA tutorial that explains the `scheduler_target_fmax_mhz` attribute and its effect on the performance of kernels.
...
2. By using the `-Xsclock=<clock target in Hz/KHz/MHz/GHz or s/ms/us/ns/ps>` option in the ahls command to direct the compiler to compile all kernels globally at a specific fMAX target.
